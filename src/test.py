...
ROOT_EXP_DIR = Path(__file__).resolve().parents[1] / "experiments_FL"
...
def test(cfg: Namespace) -> None:
    ...
    for batch_idx, data in enumerate(dataloader, start=1):
        ...
        #     name=exp_dir / f"out/test_{batch_idx}.bmp",
        # )

        save_imgs(
            imgs=out.unsqueeze(0),
            to_size=(3, 768, 1280),
            name=exp_dir / f"out/test_{str(batch_idx).zfill(2)}.png",
        )
...
if __name__ == "__main__":
    ...
    args.config = '/home/yaso/FL_CAE/configs/test.yaml'
